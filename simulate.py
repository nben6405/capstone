...
# Grid resolution — set these directly
GRID_COLS = 4
GRID_ROWS = 4

# Tile size is derived automatically
TILE_W = SCINT_WIDTH  / GRID_COLS   # cm per tile horizontally
TILE_H = SCINT_HEIGHT / GRID_ROWS   # cm per tile vertically

# Layer heights (cm above bottom imaging grid)
Z_TOP_PADDLE  = 30.0  # top trigger paddle
Z_BOT_PADDLE  = 20.0  # second trigger paddle  
Z_IMAGING_BOT = 0.0   # bottom imaging grid — reference plane, always 0
...
BLOCK_Z     = 15.0    # cm  height above bottom grid
BLOCK_ATTEN = 0.85    # fraction of muons blocked
...
def generate_muons(n, rng):
    ...
    Each muon is defined by:
      - (x0, y0) : entry point at the top paddle (cm)
      - (theta)  : zenith angle from vertical (radians)
      - (phi)    : azimuth angle (radians)
    
    Zenith distribution follows cos^2(theta) — most muons come
    nearly straight down, fewer come in at steep angles.
    Cutoff at 70 degrees — beyond that flux is negligible.
    """
    
    # Entry point — uniform over scintillator active area
    x0 = rng.uniform(0, SCINT_WIDTH,  n)
    y0 = rng.uniform(0, SCINT_HEIGHT, n)
    
    # Zenith angle — sample from cos^2 distribution
    # Rejection sampling: draw uniform theta, accept with prob cos^2(theta)
    theta = []
    while len(theta) < n:
        candidate = rng.uniform(0, np.radians(70), n)
        accept_prob = np.cos(candidate) ** 2
        mask = rng.uniform(0, 1, n) < accept_prob
        theta.extend(candidate[mask].tolist())
    theta = np.array(theta[:n])
    
    # Azimuth — uniform 0 to 2pi (no preferred horizontal direction)
...
def track_position_at_z(x0, y0, theta, phi, z_start, z_target):
    """
    Given a muon starting at (x0, y0) at height z_start, travelling
    at angles (theta, phi), return its (x, y) position at z_target.
    
    As the muon travels downward, its horizontal position drifts
    by: drift = (z_start - z_target) * tan(theta)
    """
    dz   = z_start - z_target        # vertical distance travelled
    drift = dz * np.tan(theta)        # horizontal drift magnitude
    
    x = x0 + drift * np.cos(phi)     # x component of drift
    y = y0 + drift * np.sin(phi)     # y component of drift
    
    return x, y

def check_block(x0, y0, theta, phi, rng):
    """
    For each muon, compute where it passes through the block's height
    and check if it hits the block. If it does, apply attenuation.
    
    Returns a boolean mask — True means the muon SURVIVED (was not blocked).
    """
    
    # Find where each muon track is at the block's height
    ...
    # Check if that position is inside the block rectangle
    in_block = (
        (x_block >= BLOCK_X) & (x_block <= BLOCK_X + BLOCK_W) &
        (y_block >= BLOCK_Y) & (y_block <= BLOCK_Y + BLOCK_H)
    )
    
    # For muons inside the block, roll against attenuation probability
    # rng.random() < BLOCK_ATTEN means the muon is absorbed
...
def pos_to_tile(x, y):
    """
    Convert a physical (x, y) position in cm to a (row, col) tile index.
    Returns (-1, -1) if the position is outside the active area.
    """
    
    col = (x / TILE_W).astype(int)
    row = (y / TILE_H).astype(int)
    
    # Check within active area
    in_bounds = (
        (x >= 0) & (x < SCINT_WIDTH) &
        (y >= 0) & (y < SCINT_HEIGHT)
    )
    
    # Clamp to grid edges (handles floating point at boundaries)
...
def simulate_A(x0, y0, theta, phi, survived, rng):
    ...
    A valid event requires:
      - Muon survived the block
      - Muon lands within the active area of the bottom imaging grid
    
    Position is determined entirely by where the muon hits the bottom grid.
    The top paddles have no spatial information — they just confirm a muon
    passed through. Any muon that hits anywhere on the top paddle is a
    valid trigger, so there's no top-grid position check.
    """
    
    hit_map = np.zeros((GRID_ROWS, GRID_COLS), dtype=int)
    total_events = 0
    
    # Only process muons that survived the block
    mask = survived
    
    # Find where surviving muons hit the bottom imaging grid
    x_bot, y_bot = track_position_at_z(
        x0[mask], y0[mask], theta[mask], phi[mask],
        z_start  = Z_TOP_PADDLE,
    ...
    # Valid events — muon landed within active area
...
def simulate_B(x0, y0, theta, phi, survived, rng):
    ...
    A valid event requires:
      - Muon survived the block
      - Muon lands within active area on BOTH grids
      - The tile fired on the top grid matches the tile fired on the bottom grid
    
    This last condition is where Config B differs from Config A.
    Angled muons that drift across a tile boundary between the two layers
    fire different tiles on top and bottom — these fail the coincidence
    and are rejected. This tightens the angular acceptance but loses counts.
    
    The hit is recorded on the bottom grid tile.
    """
    ...
    # Where muon hits the TOP imaging grid
    x_top, y_top = track_position_at_z(
        x0[mask], y0[mask], theta[mask], phi[mask],
        z_start  = Z_TOP_PADDLE,
        z_target = Z_TOP_PADDLE       # top grid is at the same height as top paddle
    )
    
    # Where muon hits the BOTTOM imaging grid
    x_bot, y_bot = track_position_at_z(
        x0[mask], y0[mask], theta[mask], phi[mask],
        z_start  = Z_TOP_PADDLE,
        z_target = Z_IMAGING_BOT
    ...
    # Tile indices for both grids
    row_top, col_top = pos_to_tile(x_top, y_top)
    row_bot, col_bot = pos_to_tile(x_bot, y_bot)
    
    # Valid events — in bounds on both grids AND same tile on both
...
def run_snr_test(config_fn, x0, y0, theta, phi, rng, 
    ...
    SNR = (mean open tile rate - mean shadow tile rate) / std(open tile rate)
    
    We classify tiles as shadow or open based on whether the block's
    projection falls on that tile at the bottom grid.
    """
...
def plot_results(res_a, res_b):
    """
    res_a, res_b: tuples of
      (hit_map, total_events, muons_to_detect, snr_history, muon_history)
    """
    ...
    # Real-world imaging time estimate
    # Muon flux ~1 per cm^2 per minute at sea level
    FLUX      = 1.0   # muons / cm^2 / min
    area      = SCINT_WIDTH * SCINT_HEIGHT
    rate      = FLUX * area   # muons per minute entering detector
    time_a    = mu_a / rate
    time_b    = mu_b / rate
    ...
    # ── Helper: draw block outline on a tile-axes plot ──
    def draw_block(ax):
    ...
    lines = [
        ...
        ('#aaa',    '  Block size',       f'{BLOCK_W}×{BLOCK_H} cm'),
        ('#aaa',    '  Block atten',      f'{BLOCK_ATTEN*100:.0f}%'),
    ]
    ...
    plt.savefig('/mnt/user-data/outputs/muography_sim_results.png',
                dpi=150, bbox_inches='tight', facecolor='#0d1117')
    print("Saved: muography_sim_results.png")
    plt.show()
